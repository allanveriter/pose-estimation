...
from ray2world import ray2world
...
class CameraFeed():
    ...
    def __repr__(self):
        return f"Camera name: {self.camera_name}"  # what shows up in lists
...
def process_camera_data(list_of_lists_of_lines):
    ...
    for i, lines in enumerate(list_of_lists_of_lines):
        ...
        if len(valid_lines) < 1:
            # Case 1: no valid line, return None
            world.append(None)
            #print("not enough valid lines")

        elif len(valid_lines) == 1:
            # Case 2: One valid lines, return its intersection with a plane
            world.append(ray2world(valid_lines))

        else:
            print("more than 1 lines, not possibel fix it???")
