...
def pixel2ray2(pixel, camera_matrix, rvec, tvec):
    start = time.perf_counter()
    #camera_matrix has already been inversed to save time
    N = CoordSys3D('N')
    pixel_homogenous = sp.Matrix([*pixel, 1])

    line_direction_camera = camera_matrix * pixel_homogenous  # same as ray

    line_point_camera = sp.Matrix([0, 0, 0])  # A point on the line in camera coordinates
    end = time.perf_counter()
    print("pixel2ray ", 1000*(end-start))
    ...
    #evaluate = False solves that problem
...
def pixel2ray(pixel, camera_matrix, rvec, tvec):
    ...
    pixel_homog = np.array([*pixel, 1.0])

    # Ray in camera coordinates
    line_direction_camera = camera_matrix @ pixel_homog
    line_point_camera = np.zeros(3)
    ...
    return line_point_object, line_point_object + line_direction_object
